...
"democontrol.install_dir" : "/root"
		...
		/var/lib/ambari-server/resources/scripts/configs.sh set $AMBARI_HOST $CLUSTER_NAME control-config control-config
       	...
       	TASKID=$(curl -u admin:admin -H "X-Requested-By:ambari" -i -X PUT -d '{"RequestInfo": {"context" :"Install Demo Control"}, "Body": {"ServiceInfo": {"maintenance_state" : "OFF", "state": "INSTALLED"}}}' http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/services/ALARM_FATIGUE_DEMO_CONTROL | grep "id" | grep -Po '([0-9]+)')
...
PAYLOAD="{\"name\":\"afeventavro\",\"type\":\"avro\",\"schemaGroup\":\"alarmfatigue\",\"description\":\"alarmfatigue\",\"evolve\":true,\"compatibility\":\"BACKWARD\"}"
...
echo $PAYLOAD
	...
	curl -u admin:admin -i -H "content-type: application/json" -d "$PAYLOAD" -X POST http://$AMBARI_HOST:7788/api/v1/schemaregistry/schemas/afeventavro/versions
